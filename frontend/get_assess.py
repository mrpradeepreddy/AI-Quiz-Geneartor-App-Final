...
def get_assessments():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Attempt silent refresh via /auth/refresh using current token
            try:
                headers = {"Authorization": f"Bearer {st.session_state.token}"}
                refresh = requests.post(f"{API_BASE_URL}/auth/refresh", headers=headers)
                if refresh.status_code == 200:
                    st.session_state.token = refresh.json().get("access_token")
                    headers = {"Authorization": f"Bearer {st.session_state.token}"}
                    response = requests.get(f"{API_BASE_URL}/assessments/", headers=headers)
                    if response.status_code == 200:
                        return response.json()
