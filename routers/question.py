...
from auth.jwt import get_current_user, require_recruiter, require_student
...
async def create_questions_bulk(
    ...
    """Create multiple questions at once (recruiter only)."""
    created_questions = []
...
async def add_choice_to_question(
    ...
    """Add a choice to a question (recruiter only)."""
    question = db.query(Question).filter(Question.id == question_id).first()
    if not question:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def update_choice(
    ...
    """Update a choice (recruiter only)."""
...
async def delete_choice(
    ...
    """Delete a choice (recruiter only)."""
    choice = db.query(Choice).filter(Choice.id == choice_id).first()
    if not choice:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
