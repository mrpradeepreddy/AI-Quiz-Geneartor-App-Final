...
async def login(user_credentials: UserLogin, db: Session = Depends(get_db)):
    ...
    user = authenticate_user(db, user_credentials.username or user_credentials.email, user_credentials.password)
    if not user:
        raise HTTPException(
...
async def register(user_data: UserCreate,background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    # Auto-generate recruiter_code for recruiters
    if db_user.role and db_user.role.lower() in ["recruiter", "admin"]:
        # Prefer short code if you want, or use full UUID
        db_user.recruiter_code = str(uuid.uuid4())
